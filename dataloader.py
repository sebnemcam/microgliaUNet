...
directory= "/lustre/groups/iterm/sebnem/runs/30.06_18:50/"
'''
...
for train_val_data, test_data in kfold.split(data['image'],data['segmentation']):

    test_set = CacheDataset(test_data, dic_transforms)
    test_fold += 1

    for train_data, val_data in kfold.split(train_val_data['image'],data['segmentation']):
            ...
            if epoch % val_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    if metric > best_metric[fold]:
                        ...
                        best_metric_epoch[fold] = epoch +1
                        print(f"Fold {fold} \nBest Dice {best_metric[fold]} \nat epoch{best_metric_epoch[fold]}")
    ...
    plt.show()
    plt.savefig(f"/lustre/groups/iterm/sebnem/LearningCurvesTestFold{test_fold}.png")
